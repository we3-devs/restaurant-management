...
/**
 * The live `suppliers`/`purchase_orders`/procurement tables were provisioned by an
 * older (pre-TypeORM) schema and never actually migrated to match the current
 * entities, even though CreateSupplierManagementTables1770500000000 and
 * CreateProcurementTables1770600000000 are recorded as applied. All affected
 * tables were empty (0 rows), so this reconciles the live schema by dropping the
 * stale tables and recreating them exactly as the current entities define them.
 */
export class ReconcileSupplierProcurementSchema1771300000000
  implements MigrationInterface
{
  name = 'ReconcileSupplierProcurementSchema1771300000000';
