...
export class AnalyticsService {
  ...
  async overview(user: User, query: AnalyticsQueryDto) {
    ...
    const [row, salesRow] = await Promise.all([
      base.clone().select('COUNT(*)', 'orders').addSelect('COALESCE(SUM(o.due_amount),0)', 'outstanding').addSelect('COUNT(DISTINCT o.customer_id) FILTER (WHERE o.customer_id IS NOT NULL)', 'customers').getRawOne(),
      paidBase.clone().select('COUNT(*)', 'paidOrders').addSelect('COALESCE(SUM(o.grand_total),0)', 'grossSales').addSelect('COALESCE(SUM(o.subtotal - o.discount_amount + o.service_charge_amount + o.tax_amount - o.refunded_amount),0)', 'netSales').addSelect('COALESCE(SUM(o.discount_amount),0)', 'discounts').addSelect('COALESCE(SUM(o.tax_amount),0)', 'tax').addSelect('COALESCE(SUM(o.service_charge_amount),0)', 'serviceCharge').addSelect('COALESCE(SUM(o.refunded_amount),0)', 'refunds').addSelect('COALESCE(SUM(o.loyalty_discount_amount),0)', 'loyaltyDiscount').getRawOne(),
    ]);
    Object.assign(row, salesRow);
    const orders = n(row?.orders);
    const trend = await paidBase.clone().select("TO_CHAR(o.created_at, 'YYYY-MM-DD')", 'date').addSelect('COUNT(*)', 'orders').addSelect('COALESCE(SUM(o.grand_total),0)', 'revenue').groupBy('date').orderBy('date').getRawMany();
    const [sources, types, payments, itemRow] = await Promise.all([
      paidBase.clone().select('o.order_source', 'name').addSelect('COUNT(*)', 'orders').addSelect('COALESCE(SUM(o.grand_total),0)', 'revenue').groupBy('o.order_source').getRawMany(),
    ...
    return { range: { from: scope.from, to: scope.to }, kpis: { grossSales:n(row?.grossSales), netSales:n(row?.netSales), orders, averageOrderValue: n(row?.paidOrders) ? n(row?.grossSales)/n(row?.paidOrders) : 0, customers:n(row?.customers), itemsSold:n(itemRow?.itemsSold), discounts:n(row?.discounts), tax:n(row?.tax), serviceCharge:n(row?.serviceCharge), refunds:n(row?.refunds), outstanding:n(row?.outstanding), loyaltyDiscount:n(row?.loyaltyDiscount) }, trend: trend.map(x=>({date:x.date,orders:n(x.orders),revenue:n(x.revenue)})), orderMix: { sources: sources.map(x=>({name:x.name,orders:n(x.orders),revenue:n(x.revenue)})), types: types.map(x=>({name:x.name,orders:n(x.orders)})) }, paymentMix: payments.map(x=>({name:x.name,amount:n(x.amount)})) };
  }

  async products(user: User, query: AnalyticsQueryDto) {
    const scope = await this.scope(user, query);
    const qb = this.apply(this.orders.manager.createQueryBuilder().from('order_items','i').innerJoin('orders','o','o.id=i.order_id').innerJoin('foods','f','f.id=i.food_id').leftJoin('food_categories','c','c.id=f.food_category_id'),'o',scope).andWhere("o.status != 'cancelled'").andWhere("o.payment_status = 'paid'");
  ...
  async customers(user: User, query: AnalyticsQueryDto) {
    ...
    const spendRow = await base.clone().andWhere("o.payment_status = 'paid'").select('COALESCE(SUM(o.grand_total),0)','spend').getRawOne<{ spend: string }>();
    const row=await base.clone().select('COUNT(DISTINCT o.customer_id)','customers').addSelect('COUNT(*)','orders').addSelect(`COUNT(DISTINCT o.customer_id) FILTER (WHERE NOT ${prior})`,'newCustomers').addSelect(`COUNT(DISTINCT o.customer_id) FILTER (WHERE ${prior})`,'returningCustomers').setParameters({ from: scope.from, ...(scope.outlets === ALL_OUTLETS ? {} : { outletIds: Array.isArray(scope.outlets) ? scope.outlets : [scope.outlets] }) }).getRawOne();
    ...
    return { kpis:{totalCustomers,orders,newCustomers:n(row?.newCustomers),returningCustomers,repeatRate:totalCustomers?returningCustomers/totalCustomers*100:0,averageSpend:totalCustomers?n(spendRow?.spend)/totalCustomers:0,averageVisits:totalCustomers?orders/totalCustomers:0}, trend:trendRows.map((r:{date:string;newCount:string;returningCount:string})=>({date:String(r.date).slice(0,10),newCount:n(r.newCount),returningCount:n(r.returningCount)})) };
  }
}
