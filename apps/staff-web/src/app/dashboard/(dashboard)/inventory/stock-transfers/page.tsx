...
const columns: ColumnDef<StockTransfer>[] = [
  ...
  { accessorKey: "transferDate", header: "Date" },
  {
    id: "status",
    header: "Status",
    cell: ({ row }) => (
  ...
  return (
    <div className="page-shell space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">Stock Transfers</h1>
              ...
              <TableRow key={row.id}>
                {row.getVisibleCells().map((cell) => (
                  <TableCell key={cell.id}>
                    <Link href={`/dashboard/inventory/stock-transfers/${row.original.id}`} className="block">
