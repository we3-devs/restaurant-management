...
function Breadcrumb({ orderNumber }: { orderNumber: string }) {
  return (
    <div className="flex items-center gap-1.5 text-xs font-medium tracking-wide text-muted-foreground uppercase">
      ...
      <Link href="/dashboard/overview/invoices" className="hover:text-foreground">
        Invoices
      </Link>
      <ChevronRightIcon className="size-3.5" />
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-dashed p-8 text-center">
          ...
          <Link href={`/dashboard/overview/orders/${orderId}`} className="mt-4 inline-block text-sm text-primary hover:underline">
            View order details
          </Link>
        </div>
      </div>
