...
export const REPORT_ROW_LINK: Partial<Record<ReportType, { column: string; href: (row: Record<string, unknown>) => string | null }>> = {
  // /dashboard/overview/orders/:id is the hub for an order — it already shows the
  // order items, the bill receipt, the payment summary and the invoice.
  sales: { column: "orderNumber", href: (row) => (id(row.orderId) ? `/dashboard/overview/orders/${id(row.orderId)}` : null) },
  orders: { column: "orderNumber", href: (row) => (id(row.orderId) ? `/dashboard/overview/orders/${id(row.orderId)}` : null) },
  payments: { column: "orderNumber", href: (row) => (id(row.orderId) ? `/dashboard/overview/orders/${id(row.orderId)}` : null) },
  "kitchen-performance": { column: "orderNumber", href: (row) => (id(row.orderId) ? `/dashboard/overview/orders/${id(row.orderId)}` : null) },
  "purchase-orders": { column: "poNo", href: (row) => (id(row.poId) ? `/dashboard/purchasing/purchase-orders/${id(row.poId)}` : null) },
  "stock-movements": {
    column: "documentNo",
    href: (row) => {
