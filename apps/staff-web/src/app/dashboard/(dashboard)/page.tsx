...
export default function DashboardPage() {
  ...
  if (isLoadingOutlets || queries.some((query) => query.isLoading)) return <div className="dashboard-page"><div className="dash-loading">Loading dashboard…</div></div>
  if (queries.some((query) => query.isError) || !stats.data || !charts.data || !orders.data || !tables.data || !kitchen.data || !analytics.data) return <div className="dashboard-page"><ErrorState retry={retry} /></div>
  ...
  const trend = charts.data.revenueTrend; const maxRevenue = Math.max(...trend.map((point) => point.grandTotal), 1)
  const points = trend.map((point, index) => `${trend.length === 1 ? 400 : (index / (trend.length - 1)) * 800},${190 - (point.grandTotal / maxRevenue) * 160}`).join(" ")
  ...
  return <div className="dashboard-page page-shell">
    ...
    <div className="dash-main-grid">
      <Panel className="sales-panel"><Heading title="Sales Overview" subtitle={`${trend.length} revenue data point${trend.length === 1 ? "" : "s"}`} /><div className="chart-wrap"><div className="chart-y"><span>{money(maxRevenue)}</span><span>{money(maxRevenue * .75)}</span><span>{money(maxRevenue * .5)}</span><span>{money(maxRevenue * .25)}</span><span>$0</span></div><svg viewBox="0 0 800 220" preserveAspectRatio="none" aria-label="Sales overview chart"><defs><linearGradient id="salesFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#f5b51b" stopOpacity=".42" /><stop offset="1" stopColor="#f5b51b" stopOpacity=".03" /></linearGradient></defs>{points && <><polyline points={`${points} 800,220 0,220`} fill="url(#salesFill)" /><polyline points={points} fill="none" stroke="#f5b51b" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></>}</svg><div className="chart-x">{trend.map((point) => <span key={point.date}>{point.date}</span>)}</div></div></Panel>
      <Panel><Heading title="Recent Orders" subtitle="Latest orders across the active outlet" /><div className="dash-table"><div className="dash-table-head"><span>Table #</span><span>Customer</span><span>Time</span><span>Status</span></div>{orders.data.data.map((order) => <div className="dash-table-row" key={order.id}><span>{order.tableName ?? order.orderNumber}</span><span>{order.customerName ?? `Order ${order.orderNumber}`}</span><span>{time(order.createdAt)}</span><Status>{order.status}</Status></div>)}</div></Panel>
    </div>
    <Panel className="items-panel"><Heading title="Top Selling Items" action="" /><div className="item-grid">{bestSelling.map((item) => <Link href={`/dashboard/foods/${item.foodId}`} className="selling-item" key={item.foodId}><div className="item-emoji">🍽️</div><div><strong>{item.foodName}</strong><small>{item.quantitySold} sold · {money(item.revenue)}</small></div></Link>)}</div></Panel>
    <div className="dash-main-grid lower-grid"><Panel><Heading title="Table Occupancy" subtitle="Live status of tables in the restaurant" /><div className="occupancy"><div className="donut" style={{ background: `conic-gradient(#f5b51b 0 ${tableTotal ? (occupied / tableTotal) * 100 : 0}%,#25354e 0)` }}><strong>{tableTotal ? Math.round((occupied / tableTotal) * 100) : 0}%</strong><span>Occupied</span></div><div className="legend"><span><i className="green" />Available <b>{tableCounts.available ?? 0}</b></span><span><i className="gold" />Occupied <b>{occupied}</b></span><span><i className="blue" />Cleaning <b>{tableCounts.cleaning ?? 0}</b></span><span><i className="red" />Reserved <b>{tableCounts.reserved ?? 0}</b></span></div><div className="table-grid">{tables.data.data.map((table) => <span key={table.id} className={table.status}>{table.name}<small>{table.status}</small></span>)}</div></div></Panel><Panel><Heading title="Kitchen Queue" subtitle="Open and preparing tickets, oldest first" /><div className="queue">{queue.map((ticket) => { const item = ticket.items?.[0]?.orderItem?.food?.name ?? `Order ${ticket.order?.orderNumber ?? ticket.orderId}`; const table = ticket.order?.tableSession?.diningTable?.name ?? "No table"; return <div className="queue-row" key={ticket.id}><i className={ticket.status === "in_progress" ? "gold-dot" : "green-dot"} /><strong>{table}</strong><span>—</span><span>{item}</span><time>{time(ticket.createdAt)}</time><Status>{ticket.status === "in_progress" ? "Preparing" : "Open"}</Status></div> })}</div></Panel></div>
    <Panel className="activity-panel"><Heading title="Today&apos;s domain activity" subtitle="Real records from purchasing, reservations, staff, loyalty and system activity" /><div className="activity-grid">{activityLabels.map(([key, Icon]) => <div className="activity" key={key}><div className="activity-icon"><Icon /></div><span>{key.replaceAll("-", " ")}<b>{analytics.data.domains[key]?.meta.total ?? 0}</b></span></div>)}</div></Panel>
  </div>
}
