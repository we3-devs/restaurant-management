...
export default function StaffTablesPage() {
  ...
  return <div className="flex flex-1 flex-col gap-3">
    ...
    {!effectiveOutletId ? <p className="text-sm text-muted-foreground">Select an outlet to start.</p> : !activeOrderId ? <div className="flex-1 overflow-y-auto"><FloorBoard outletId={effectiveOutletId} basePath={BASE_PATH} showHeader={false} /></div> : isCancelledOrder ? <div className="flex flex-1 flex-col items-center justify-center gap-3 text-center"><p className="text-sm font-medium">This order was cancelled</p><p className="text-sm text-muted-foreground">It can no longer be edited.</p><Button onClick={() => router.push(BASE_PATH)}>Back to tables</Button></div> : <LocalCartProvider orderId={activeOrderId}><div className="flex flex-1 flex-col gap-3 overflow-hidden"><CategoryTabs categoryId={categoryId} onSelect={setCategoryId} /><FoodGrid categoryId={categoryId} /><FloatingCart orderId={activeOrderId} basePath={BASE_PATH} /></div></LocalCartProvider>}
