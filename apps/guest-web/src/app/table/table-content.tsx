		...
		return (
			<div className="flex min-h-screen items-center justify-center bg-slate-50 px-6 text-center">
				<div>
					...
					<p className="mt-1 text-sm text-slate-500">Scan the QR code on your table to start.</p>
				</div>
			</div>
		);
	}
			...
			<div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-6 text-center">
				<div className="max-w-sm space-y-2">
					...
					<p className="text-sm text-slate-500">
						Verify your number to join Table {tableCode} — or use the printed menu and order with a server.
					</p>
				</div>
						...
						{members.map((m) => (
							...
							>
								<div className="min-w-0">
									...
									{m.phone && <p className="truncate text-xs text-slate-500">{m.phone}</p>}
								</div>
								<button
