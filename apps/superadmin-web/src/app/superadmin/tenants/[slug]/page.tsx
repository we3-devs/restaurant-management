...
export default function TenantDetailPage() {
  ...
  const [backfillResult, setBackfillResult] = useState<{ refreshed: string[]; message?: string } | null>(null)
  ...
  async function backfillAnalytics() {
    ...
    try {
      const result = await api(`/analytics/daily/backfill?to=${today()}`, {
        method: "POST",
  ...
  return <div className="mx-auto w-full max-w-6xl space-y-6">
    ...
    <section className="rounded-xl border border-primary/20 bg-primary/[0.03] p-4"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-semibold">Historical analytics</h2><p className="mt-1 max-w-2xl text-sm text-muted-foreground">Parse this tenant&apos;s existing orders into daily analytics snapshots. The selected tenant is sent explicitly, so records from other tenants are excluded.</p></div><Button onClick={() => void backfillAnalytics()} disabled={backfillRunning || !tenant.isActive}>{backfillRunning ? "Processing history…" : "Build all historical analytics"}</Button></div>{backfillRunning && <p className="mt-3 rounded-lg border bg-background/60 px-3 py-2 text-sm text-muted-foreground">Processing one business day at a time. Keep this page open until it finishes.</p>}{backfillResult && <p className="mt-3 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-sm"><span className="font-medium">Completed.</span> {backfillResult.message ?? `${backfillResult.refreshed.length} business days processed.`}</p>}</section>
